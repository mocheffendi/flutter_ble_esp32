...
import 'package:flutter/services.dart' show PlatformException, Uint8List;
import 'package:ble_esp32/app/constant/constant.dart'
    show maxConnectionTimeOut, maxTryReconnect;
...
class BluetoothData {
  ...
  Future<void> initBluetooth() async {
    ...
    FlutterBluetoothSerial.instance
      ...
      ctrl.refreshDeviceList();
      debugPrint(
          '[bluetooth_data] onStateChanged: ${ctrl.isBluetoothActive.value}');
    });
  }
    ...
    if (device == null) {
      ...
      if (!isConnected!) {
        await BluetoothConnection.toAddress(device?.address).then((conn) {
          ...
          showGetxSnackbar(
              'Failed to connect', 'Cannot connect, exception occurred');
  ...
  void reConnect() {
    Future.delayed(const Duration(seconds: 5)).then((value) {
      if (ctrl.isAutoReconnect.isTrue &&
          bluetoothState == BluetoothState.STATE_ON &&
          _isConnectionLost &&
          _reconnectCounter < maxTryReconnect) {
        _reconnectCounter++;
        debugPrint(
            '[bluetooth_data] auto reconnect active, reconnecting #$_reconnectCounter/$maxTryReconnect');
        ctrl.refreshLogs(
            text:
                'Auto reconnect active, reconnecting #$_reconnectCounter/$maxTryReconnect');
      ...
      (Timer timer) {
        ...
        if (start == 0) {
          ...
          if (ctrl.isConnecting.isTrue) {
            ...
            ctrl.refreshLogs(
                text: 'Failed to connect. Connection time out!',
