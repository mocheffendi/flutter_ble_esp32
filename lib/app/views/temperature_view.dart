...
import 'package:ble_esp32/app/constant/constant.dart'
    show AppColors, maxCommandHistoryCount;
import 'package:ble_esp32/utils.dart' show SourceId;
import 'package:get/get.dart';
import 'package:grouped_list/grouped_list.dart'
    show GroupedListOrder, GroupedListView;
import 'package:intl/intl.dart' show DateFormat;
import 'package:ble_esp32/bluetooth_dart.dart' show BluetoothData;
import 'package:ble_esp32/main.dart' show ctrl, listScrollController;
import 'package:ble_esp32/app/views/chat_box.dart' show ChatBox;
import 'package:ble_esp32/app/models/messages.dart' show Message;
...
class TemperatureView extends StatelessWidget {
  ...
  Widget buildStandardLogViews() {
    final List<Text> logList = ctrl.logs.value.reversed.map((message) {
      return Text(
        '${DateFormat('Hms').format(message.logTime)}: ${message.text}',
        style: TextStyle(
            color: message.whom == SourceId.hostId
                ? Colors.blue
                : message.whom == SourceId.clientId
                    ? Colors.black
                    : Colors.green),
      );
    }).toList();
      ...
      child: NotificationListener(
        ...
        child: GroupedListView<Message, DateTime>(
          ...
          itemBuilder: (context, Message message) {
            return ChatBox(
                key: UniqueKey(),
                deviceId: message.whom,
                teks: message.text,
                logTime: DateFormat('Hms').format(message.logTime));
          },
        ),
      ),
    );
  ...
  void sendMessage() {
    ...
    if (text.isNotEmpty) {
      try {
        ...
        ctrl.refreshLogs(
            sourceId: SourceId.hostId, text: 'send message error: $e');
      }
    }
  }
    ...
    return List.generate(ctrl.cmdListHistory.length, (index) {
      return Row(
        children: [
          SizedBox(
              height: 30,
              child: buildHistoryCommandContainer(ctrl.cmdListHistory[index])),
          const SizedBox(
            width: 6,
          )
        ],
