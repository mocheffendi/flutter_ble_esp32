...
import 'package:flutter/material.dart'
    show
        ...
        TickerProvider,
        debugPrint;
import 'package:ble_esp32/utils.dart' show SourceId, getCurrentDateTime;
...
class Controller extends GetxController {
  ...
  final logs = RxList<Message>([])
      .obs; // work too. to insert data --> logs.value.add() and then refresh --> logs.refresh()
  ...
  void refreshDeviceList() {
    ...
    deviceItems.refresh();
    debugPrint(
        '[global_controller] controller device items: ${deviceItems.value.length}');
  }

  void refreshLogs(
      {SourceId sourceId = SourceId.statusId, required String text}) {
    logs.value.add(Message(
        whom: sourceId,
        text: text,
        logTime: DateTime.tryParse(getCurrentDateTime())!));
    logs.refresh();
    ...
    tabController.addListener(() {
      if (tabController.indexIsChanging ||
        ...
        if (selectedTabIndex.value != 1) {
          // hide keyboard if tab position is not equal 1 / not in data logs view
  ...
  void resetLogTimer() {
    ...
    logTimer
        ?.cancel(); // if log timer not null and still active, stop current log timer
  }
